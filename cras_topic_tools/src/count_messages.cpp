...
void CountMessagesComponent::reportCb() {
  ...
  RCLCPP_INFO(
    ...
    this->countSinceLastReport, (this->get_clock()->now() - *this->lastReportStamp).seconds(), this->count,
    this->bytes);
...
void CountMessagesComponent::discoverTopicAndSubscribe() {
  ...
  if (type.empty()) {
    RCLCPP_INFO_THROTTLE(
      this->get_logger(), *this->get_clock(), 1000, "Waiting for topic %s", this->resolvedTopic.c_str());
    return;
  }
  ...
  this->sub = ::cras::create_generic_subscription(
    *this, this->topic, type, this->qosProfile, std::bind(&CountMessagesComponent::cb, this, std::placeholders::_1),
    opts);
