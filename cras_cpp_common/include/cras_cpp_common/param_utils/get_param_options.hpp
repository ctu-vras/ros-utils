...
struct GetParamOptions
{
  ...
  ToStringFn<ParamServerType> paramToStr = [](const ParamServerType& s){ return ::cras::to_string(s); };

  //! \brief A function that converts ResultType values to string for use in log messages.
  ToStringFn<ResultType> resultToStr = [](const ResultType& s){ return ::cras::to_string(s); };

  //! \brief A function converting ParamServerType values to ResultType.
  ToResultFn toResult = [](const ParamServerType& v){ return static_cast<ResultType>(v); };
  ...
  GetParamOptions& operator=(const GetParamOptions<R1, P1>& other)
  {
   ...
   * \return This.
   */
  GetParamOptions& operator=(const GetParamOptions<ResultType, ParamServerType>& other)
  {
    this->printMessages = other.printMessages;
    this->printDefaultAsWarn = other.printDefaultAsWarn;
    this->throwIfConvertFails = other.throwIfConvertFails;
  ...
  GetParamOptions<ResultType, NewParamServerType> asType(
    typename GetParamOptions<ResultType, NewParamServerType>::ToResultFn newToResult = 
      [](const NewParamServerType& v){ return static_cast<ResultType>(v); },
    ToStringFn<NewParamServerType> newParamToStr = [](const NewParamServerType& s){ return ::cras::to_string(s); },
    typename GetParamOptions<ResultType, NewParamServerType>::ToParamFn newToParam =
