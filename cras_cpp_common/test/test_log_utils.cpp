...
TEST(MemoryLogUtils, Basic)  // NOLINT
{
  ...
  logIf->clear(); RCLCPP_ERROR(log, "a"); ASSERT_EQ(1u, logIf->getMessages().size());
  msg = logIf->getMessages().front();
  EXPECT_EQ(__FILE__, msg.file); EXPECT_EQ(__LINE__ - 2, msg.line);  // This line has to be right after the log call
  EXPECT_EQ("a", msg.msg); EXPECT_EQ(rcl_interfaces::msg::Log::ERROR, msg.level);
  EXPECT_NEAR(clk.now().seconds(), cras::float_secs(msg.stamp), 1.0); EXPECT_EQ("test", msg.name);
  EXPECT_EQ(__func__, msg.function);

  logIf->clear(); RCLCPP_FATAL(log, "a"); ASSERT_EQ(1u, logIf->getMessages().size());
  msg = logIf->getMessages().front();
  EXPECT_EQ(__FILE__, msg.file); EXPECT_EQ(__LINE__ - 2, msg.line);  // This line has to be right after the log call
  ...
  logIf->clear(); for (size_t i = 0; i < 3; ++i) {
    RCLCPP_INFO_ONCE(log, "%zu", i);
}
